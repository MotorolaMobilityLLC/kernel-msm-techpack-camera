...
typedef struct {
	...
	mot_lens_step park_lens_step[LENS_MAX_STAGES];
} mot_park_lens;

typedef struct {
	bool launch_lens_needed;
	mot_lens_step launch_lens_step[LENS_MAX_STAGES];
} mot_launch_lens;
...
typedef struct {
	...
	mot_actuator_hw_info actuator_info[MOT_ACTUATOR_NUM];
} mot_dev_info;
...
static mot_actuator_settings mot_actuator_list[MOT_ACTUATOR_NUM] = {
...
static mot_actuator_runtime_type mot_actuator_runtime[MOT_ACTUATOR_NUM];
static uint32_t lens_safe_pos_dac = 0;
static uint32_t lens_park_pos = 512;
static mot_lens_step lens_park_table[LENS_MAX_STAGES] = {};
...
static int mot_actuator_init_runtime(void)
{
	...
	for (i = 0; i < MOT_ACTUATOR_NUM; i++) {
		...
		CAM_WARN(CAM_ACTUATOR, "ACTUATOR NO. %d, DAC pos: %d", i, mot_actuator_runtime[i].safe_dac_pos);
...
{
	uint32_t actuator_index = mot_dev_list[mot_device_index].actuator_info[index].actuator_type;
...
static int32_t mot_actuator_move_lens_by_dac(uint32_t index, uint32_t dac_value)
{
	...
	CAM_WARN(CAM_ACTUATOR, "move lens to DAC pos: %d", dac_value);
...
{
	uint32_t cur_len_pos = mot_dev_list[mot_device_index].actuator_info[0].init_pos;
	uint32_t last_len_pos = 0;
	int stageIndex = 0;
	unsigned int consumers = 0;
	int32_t ret = 0;
	uint32_t lens_launch_pos = dac_value;
	mot_lens_step *lens_launch_table = mot_dev_list[mot_device_index].actuator_info[index].launch_lens.launch_lens_step;
	...
	while (cur_len_pos < lens_launch_pos) {
		...
		if (stageIndex == LENS_MAX_STAGES && cur_len_pos < lens_launch_pos) {
			cur_len_pos += 10;
		}
...
static int32_t mot_actuator_vib_move_lens(uint32_t index)
{
	...
	if (mot_device_index >= MOT_DEVICE_NUM || index >= MOT_ACTUATOR_NUM) {
		CAM_ERR(CAM_ACTUATOR, "INVALID device!!!");
		return -1;
	}

	consumers = mot_actuator_get_consumers();
	CAM_WARN(CAM_ACTUATOR, "consumers=%d",consumers);
	if (consumers & CLINET_CAMERA_MASK) {
		...
		if (ret == 0) {
			...
			if (ret == 0) {
				CAM_DBG(CAM_ACTUATOR, "init actuator sucess", ret);
				mot_actuator_state = MOT_ACTUATOR_INITED;
			} else {
				mot_actuator_release_cci(index);
				CAM_ERR(CAM_ACTUATOR, "init actuator failed, ret=%d!!!", ret);
			}
		} else {
	...
	if (ret == 0 && mot_actuator_state == MOT_ACTUATOR_INITED) {
		...
		if (consumers == 0) {
			...
			if (ret == 0) {
				CAM_DBG(CAM_ACTUATOR, "actuator is safe now, safe_dac_pos:%d, please start vibrating.",
...
static int32_t mot_actuator_park_lens(uint32_t index)
{
	...
	if (mot_dev_list[mot_device_index].actuator_info[index].park_lens.park_lens_needed == false) {
	...
	while (cur_len_pos > lens_park_pos) {
		...
		if (cur_len_pos <= lens_park_pos) {
			/*For skipping the last delay*/
			CAM_WARN(CAM_ACTUATOR, "Park lens done.");
			break;
		}
...
int mot_actuator_on_vibrate_start(void)
{
	...
	mot_actuator_vib_move_lens(MOT_ACTUATOR_DW9800V);
...
static int32_t mot_actuator_handle_write(uint32_t arg)
{
	...
	if (mot_actuator_state != MOT_ACTUATOR_INITED) {
		CAM_ERR(CAM_ACTUATOR, "mot actuator is not initiated!!");
		return -EINVAL;
	}
...
static void mot_actuator_handle_release(void)
{
	if (mot_actuator_state == MOT_ACTUATOR_RELEASED) {
		CAM_ERR(CAM_ACTUATOR, "Already released.");
		return;
	}

	if (mot_actuator_state == MOT_ACTUATOR_INITED) {
		CAM_DBG(CAM_ACTUATOR, "Disable AF power!");
		//regulator_disable(afVdd);
		//regulator_disable(ioVdd);
	}
	CAM_DBG(CAM_ACTUATOR, "Vsync released.");
		...
		unsigned int cmd)
{
	...
	case MOT_ACTUATOR_INIT:
		/* init */
		mot_actuator_init_runtime();
		CAM_DBG(CAM_ACTUATOR, "MOT_ACTUATOR_INIT ret:%d", rc);
		break;
	case MOT_ACTUATOR_READ:
		/* read */
		CAM_DBG(CAM_ACTUATOR, "MOT_ACTUATOR_READ ret:%d", rc);
		break;
	case MOT_ACTUATOR_WRITE:
		/* write */
		mot_actuator_handle_write(arg);
		CAM_DBG(CAM_ACTUATOR, "MOT_ACTUATOR_WRITE ret:%d", rc);
		break;
	case MOT_ACTUATOR_EXIT:
		/* exit */
		mot_actuator_handle_exit(arg);
		CAM_DBG(CAM_ACTUATOR, "MOT_ACTUATOR_EXIT ret:%d", rc);
		break;
	case MOT_ACTUATOR_RELEASE:
		/* release */
		mot_actuator_handle_release();
		CAM_DBG(CAM_ACTUATOR, "MOT_ACTUATOR_RELEASE ret:%d", rc);
		break;
	default:
		CAM_ERR(CAM_ACTUATOR, "Unknown command (%d)", cmd);
		rc = -EINVAL;
		break;
	}
...
{
	CAM_DBG(CAM_ACTUATOR, "cmd=%x, arg=%x\n", cmd, *(int *)arg);
	...
	default:
		CAM_ERR(CAM_ACTUATOR, "Unsupported cmd=%x, arg=%x\n", cmd, arg);
		CAM_DBG(CAM_ACTUATOR, "supported cmd: %x, %x, %x, %x, %x\n",
		        MOT_ACTUATOR_READ,
		        MOT_ACTUATOR_WRITE,
		        MOT_ACTUATOR_INIT,
		        MOT_ACTUATOR_EXIT,
	...
	unsigned int cmd, unsigned long arg)
{
	...
	if (copy_from_user(&cmd_data, (void __user *)arg,
		sizeof(cmd_data))) {
		CAM_ERR(CAM_ACTUATOR, "Failed to copy from user_ptr=%pK size=%zu", (void __user *)arg, sizeof(cmd_data));
		return -EFAULT;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (input) {
		CAM_WARN(CAM_ACTUATOR, "start actuator");
...
static inline ssize_t mot_actuator_park_lens_table_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	mot_lens_step *lens_park_table = mot_dev_list[mot_device_index].actuator_info[0].park_lens.park_lens_step;
	ssize_t count = 0;
	int i;
	count += sprintf(buf, "\n  knee point  step\n");
	for (i=0; i<LENS_MAX_STAGES; i++) {
